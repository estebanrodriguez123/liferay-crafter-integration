 ...
 *
 * <p>
 * All custom service methods should be put in this class. Whenever methods are added, rerun ServiceBuilder to copy their definitions into the {@link com.rivetlogic.crafter.service.CrafterLocalService} interface.
 *
 * <p>
...
public class CrafterLocalServiceImpl extends CrafterLocalServiceBaseImpl {
	...
	public List<HashMap<String, Object>> searchByMap(Map<String, String> queryParams) {
		...
		try {
			String searchServerURL = PrefsPropsUtil.getString(CrafterConstants.CRAFTER_SERVER_PROPERTY, StringPool.BLANK);
			if (Validator.isNotNull(searchServerURL)) {
				for (Map.Entry<String, String> e: queryParams.entrySet()) {
					crafterQueryParams.put(e.getKey(), new String[]{e.getValue()});
				}
				crafterQueryParams.put(CrafterConstants.CRAFTER_START, new String[]{CrafterConstants.CRAFTER_0_STRING});
				...
				if (Validator.isNotNull(contentMap.get(CrafterConstants.CRAFTER_RESPONSE))) {
					HashMap<String, Object> responseMap = (HashMap<String, Object>)contentMap.get(CrafterConstants.CRAFTER_RESPONSE);
					if (Validator.isNotNull(responseMap)) {
						List<HashMap<String, Object>> documentsMap = (List<HashMap<String, Object>>) responseMap.get(CrafterConstants.CRAFTER_DOCUMENTS);
						return documentsMap;
					}
				}
